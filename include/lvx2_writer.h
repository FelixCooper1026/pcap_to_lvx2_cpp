...
public:
    ...
    // 新增：支持不同点云数据类型的方法
    static std::vector<uint8_t> createPackageHeaderForDataType(const std::vector<uint8_t>& raw_udp_payload, 
                                                             uint32_t data_length, 
                                                             const DeviceInfo& device_info,
                                                             PointCloudDataType data_type);
    static std::vector<uint8_t> processPointCloudData(const std::vector<uint8_t>& raw_udp_payload, 
                                                     PointCloudDataType data_type,
                                                     bool convert_to_type1 = true);
};