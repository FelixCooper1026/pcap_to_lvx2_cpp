...
std::vector<uint8_t> convertPacketToEthernet(const u_char* data, uint32_t caplen, int linktype) {
    ...
    if (linktype == SLL_TYPE) {
        ...
        // 标准以太网直接读取
        result.assign(data, data + caplen);
    }
...
bool extractImuDataToCsv(const std::string& pcap_file, const std::string& csv_file, HWND hDlg) {
    ...
    while ((result = pcap_next_ex(pcap, &header, &data)) > 0) {
        ...
        if (is_linux_sll) {
            ...
            pkt_data.assign(data, data + header->caplen);
        }

        if (!pkt_data.empty()) {
            all_raw_packets.emplace_back(std::move(pkt_data));
        }
    }
    ...
    logToDialog(LogLevel::LOG_SUCCESS, "成功提取 " + std::to_string(imu_rows.size()) + " 条IMU数据到: " + csv_file);
